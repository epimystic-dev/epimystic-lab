...
Evidence (rows 1-7 run 2026-08-11; rows 8-9 run 2026-09-02; row 10 run 2026-09-04; row 11 run 2026-09-11):
...
The last four rows also encode a verdict rollup (`healthy` / `needs-attention` /
...
non-zero. All eleven tests further assert a tool-specific anchor beyond bare rc so
a false-pass from an unrelated non-zero rc (a missing `__main__.py`, an argparse
rejection of an unknown flag) cannot pass by accident. Ten anchor a
...
- **Convention B - severity-tiered.** `rc=1` for warnings only, `rc=2` for errors
  ...
  `agentmdlint`, `oraclecheck`, `elevatescan`, `delegcheck`. The last four roll
  findings into an explicit verdict (`healthy` / `needs-attention` / `unhealthy`)
  whose rc mapping is `0 / 1 / 2` - a verdict wrapper over the same severity
  tiers, not a separate convention.
...
Present on all eleven linters (`jsonlcheck`, `envcheck`, `jwtcheck`, `reqcheck`,
`licensechain`, `aicontribcheck`, `skillcheck`, `agentmdlint`, `oraclecheck`,
`elevatescan`, `delegcheck`) and prints `<tool> <version>` then exits `0`. The
`envcheck` and `reqcheck` additions landed 2026-08-12 and closed the earlier
parity gap; `agentmdlint`, `oraclecheck`, `elevatescan`, and `delegcheck` shipped
...
- ~~Add a per-tool test asserting the shared-invariants section above.~~ Done
  2026-09-09: all eleven linters carry `tests/test_shared_contract.py` locking

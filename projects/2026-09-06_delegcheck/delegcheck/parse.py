"""Text-position helpers used by rules to attach line and column to findings.

Rules operate on parsed JSON structures for structural correctness, and use
the helpers here to attach a source line and column so the finding is
...
def find_key_line_col(text: str, key: str, from_offset: int = 0) -> Tuple[int, int]:
    """Locate the first occurrence of a JSON key `"<key>"` after `from_offset`.

    Falls back to (1, 1) if the key is not found. Used to attach line info
    to findings that come from a structural walk of parsed JSON.
    """
    needle = '"' + key + '"'
...
def find_value_line_col(text: str, value_repr: str, from_offset: int = 0) -> Tuple[int, int]:

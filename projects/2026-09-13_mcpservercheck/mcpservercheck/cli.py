"""Command-line interface for mcpservercheck."""
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="mcpservercheck",
        description=(
            "Offline static linter for MCP-server registration config files "
            "(.mcp.json, mcp.json, and the mcpServers / servers blocks that MCP clients read on install)."
        ),
    )
    p.add_argument("path", nargs="?", default=".", help="Directory or file (default: current directory).")
    p.add_argument("--version", action="version", version="mcpservercheck " + __version__)
    ...
    p.add_argument("--disable", action="append", default=[], metavar="RULE_ID",
                   help="Disable a rule (repeatable). Example: --disable MSC-010")
...
def main(argv: Optional[Sequence[str]] = None,
    ...
    if args.max_files <= 0:
        stderr.write("mcpservercheck: --max-files must be > 0\n")
        return 2
    if args.max_bytes <= 0:
        stderr.write("mcpservercheck: --max-bytes must be > 0\n")
        return 2

    if not os.path.exists(args.path):
        stderr.write("mcpservercheck: path does not exist: " + args.path + "\n")
        return 2
